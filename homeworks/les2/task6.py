...
control = None
while True:
    control = input("Для выхода нажмите 'Q', чтобы продолжить 'Enter', для аналитики 'A' ").upper()
    if control == 'Q':
        break
    num += 1
    if control == 'A':
        ...
        for key, value in analytics.items():
            print(f'{key[:30]:<15}: {value}')
            print("-" * 45)
    for f in structure.keys():
        item_ = input(f'Введите "{f}" ')
        structure[f] = int(item_) if (f == 'price' or f == 'quantity') else item_
        analytics[f].append(structure[f])
    goods.append((num, structure))
